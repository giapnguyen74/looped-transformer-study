...
**Authors:** Mostafa Dehghani (University of Amsterdam), Stephan Gouws (DeepMind), Oriol Vinyals (DeepMind), Jakob Uszkoreit (Google Brain), Łukasz Kaiser (Google Brain)

*Equal contribution (Dehghani, Gouws), alphabetically by last name. Work by Dehghani performed while at Google Brain.*
...
> **Figure 1:** The Universal Transformer repeatedly refines a series of vector representations for each position of the sequence in parallel, by combining information from different positions using self-attention (see Eqn 2) and applying a recurrent transition function (see Eqn 4) across all time steps $1 \le t \le T$. We show this process over two recurrent time-steps. Arrows denote dependencies between operations. Initially, $h^0$ is initialized with the embedding for each symbol in the sequence. $h^t_i$ represents the representation for input symbol $1 \le i \le m$ at recurrent time-step t. With dynamic halting, T is dynamically determined for each position (Section 2.2).
...
$$\text{MultiHeadSelfAttention}(H^t) = \text{Concat}(head_1,...,head_k)W^O \tag{2}$$

where

$$head_i = \text{Attention}(H^t W_i^Q, H^t W_i^K, H^t W_i^V) \tag{3}$$
...
$$A^t = \text{LayerNorm}((H^{t-1} + P^t) + \text{MultiHeadSelfAttention}(H^{t-1} + P^t)), \tag{5}$$
...
$P^t \in \mathbb{R}^{m \times d}$ above are fixed, constant, two-dimensional (position, time) *coordinate embeddings*, obtained by computing the sinusoidal position embedding vectors as defined in (Vaswani et al., 2017) for the positions $1 \le i \le m$ and the time-step $1 \le t \le T$ separately for each vector-dimension $1 \le j \le d$, and summing:

$$P_{i,2j}^{t} = \sin(i/10000^{2j/d}) + \sin(t/10000^{2j/d}) \tag{6}$$

$$P_{i,2j+1}^{t} = \cos(i/10000^{2j/d}) + \cos(t/10000^{2j/d}). \tag{7}$$
...
$$p(y_{pos}|y_{[1:pos-1]},H^T) = \text{Softmax}(OH^T) \tag{8}$$
...
| Model | 10K train single | 10K train joint | 1K train single | 1K train joint |
|---|---|---|---|---|
...
|---|---|---|---|---|---|---|---|
| Best Stack-RNN (Yogatama et al., 2018) | 0.994 | 0.979 | 0.965 | 0.935 | 0.916 | 0.880 | 0.992 |
| Best LSTM (Yogatama et al., 2018) | 0.993 | 0.972 | 0.950 | 0.922 | 0.900 | 0.842 | 0.991 |
| Best Attention (Yogatama et al., 2018) | 0.994 | 0.977 | 0.959 | 0.929 | 0.907 | 0.842 | 0.992 |
...
| Model | LM control | LM dev | LM test | RC control | RC dev | RC test |
|---|---|---|---|---|---|---|
| Neural Cache (Grave et al., 2016) | 129 | 139 | - | - | - | - |
| Dhingra et al. (2018) | - | - | - | - | - | 0.5569 |
...
- Łukasz Kaiser, Aidan N. Gomez, and Francois Chollet. Depthwise separable convolutions for neural machine translation. *CoRR*, abs/1706.03059, 2017.
...
- Sainbayar Sukhbaatar, Arthur Szlam, Jason Weston, and Rob Fergus. End-to-end memory networks. In *Advances in Neural Information Processing Systems* 28, pp. 2440–2448, 2015.
...
- Wojciech Zaremba and Ilya Sutskever. Learning to execute. *CoRR*, abs/1410.4615, 2015.

## Appendix A: Detailed Schema of the Universal Transformer

> **Figure 4:** The Universal Transformer with position and step embeddings as well as dropout and layer normalization. The full architecture comprises an encoder and decoder. The encoder block applies multihead self-attention followed by a transition function, each wrapped with dropout, residual connections and layer normalization, repeated for T steps with position and timestep embeddings added to the embedded input symbols. The decoder block similarly applies masked multihead self-attention, then multihead attention over the encoder output, then the transition function, before a final softmax over output probabilities.
...
The LAMBADA task (Paperno et al., 2016) is a broad context language modeling task. In this task, given a narrative passage, the goal is to predict the last word (target word) of the last sentence (target sentence) in the passage. These passages are specifically selected in a way that human subjects are easily able to guess their last word if they are exposed to a long passage, but not if they only see the target sentence preceding the target word. Here is a sample from the dataset:
...
> **Best seed run for each task (out of 10 runs).** Error rate per task.

| Task id | 10K train single | 10K train joint | 1K train single | 1K train joint |
|---|---|---|---|---|
| 1 | 0.0 | 0.0 | 0.0 | 0.0 |
| 2 | 0.0 | 0.0 | 0.0 | 0.5 |
| 3 | 0.4 | 1.2 | 3.7 | 5.4 |
...
> **Average (±var) over all seeds (for 10 runs).** Error rate per task.
...
An example from tasks 2 (requiring two supporting facts to solve):

```
Story: Sandra journeyed to the hallway. Mary went to the bathroom.
       Mary took the apple there. Mary dropped the apple.
Question: Where is the apple?
Model's output: bathroom
```

Across the recurrent steps, the model's attention progressively focuses on the relevant supporting facts ("Mary took the apple there" and "Mary dropped the apple") that are needed to answer the question.

An example from tasks 3 (requiring three supporting facts to solve):

```
Question: Where was the apple before the bathroom?
Model's output: office
```

For this longer story (with many irrelevant facts), the visualizations show how the attention distributions sharpen across steps around the chain of facts establishing where the apple was located, illustrating the model's multi-hop reasoning over temporal states.
